...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def forward(
        ...
        # If input_ids is passed as an empty tensor we replace it with None
        if input_ids is not None and input_ids.size()[-1] < 1:
            input_ids = None

        # If there does happen to be input_ids and inputs_embeds
        # we'll append the ids embeds to the inputs_embeds
        if input_ids is not None and inputs_embeds is not None:
            inputs_embeds = torch.cat(
                [inputs_embeds, self.embed_tokens(input_ids)], dim=1
            )
            input_ids = None
