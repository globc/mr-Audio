...
class XInstructBLIP(Blip2Base):
    ...
    def __init__(
        ...
        self.finetuned = finetuned  # Path to finetuned checkpoint
    ...
    def generate(self, samples):
        ...
        if self.interleave_seconds:
            ...
            timestamp_inputs_llm = timestamp_inputs_llm.view(len(samples["timestamps"]), len(samples["timestamps"][0]),
                                                             *timestamp_inputs_llm.shape[
                                                              1:])  # batch_size X timestamps_per_batch X tokens_per_timestamp X embed_dim (= 4096)
            timestamp_atts_llm = timestamp_atts_llm.view(len(samples["timestamps"]), len(samples["timestamps"][0]),
                                                         timestamp_atts_llm.shape[
                                                             -1])  # batch_size X timestamps_per_batch X tokens_per_timestamp
    ...
    def from_config(cls, cfg):
        ...
        modalities = cfg.get("modalities", ["audio", "video"])
